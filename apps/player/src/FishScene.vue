...
import {reefTarget,reefSpecies,reefTier,reefTierProfile,reefCannon,reefFlight,reefLeadAngle} from '@new-game/game-math';
...
async function syncRoom(){
 ...
 try{const snapshot=await api<Room>('practice/reef/join',props.initialRoom?{roomId:props.initialRoom.id}:{});if(disposed)return;
  ...
  for(const fish of creatures){fish.captured=(sameRoom&&fish.captured)||!!snapshot.targets.find(t=>t.target_id===fish.id)?.captured;fish.view.visible=!fish.captured;}
  error.value='';
...
async function settleShot(shot:Projectile){
 ...
 try{
  const result=await api<{description:string;captured:boolean;award?:string;before?:{available:string;version:string};after?:{available:string;version:string}}>('practice/reef/shots',{roomId:shot.roomId,targetId:flight.targetId,requestKey:shot.requestKey,...(stage.enabled?{stake:shot.stake,aimX:flight.x,aimY:flight.y,observedAt:Math.round(shot.firedAt+flight.time*1000),firedAt:shot.firedAt,angle:flight.angle}:{})});
  if(disposed)return;
  if(result.before&&result.after)walletPresentation?.accept(result.before,result.after,result.award||'0',performance.now());
  notice.value=result.captured?'Captured!':`${reefSpecies[reefTarget(flight.targetId,0).species]} resisted the hit.`;
 ...
 }catch(e){if(!disposed){const code=(e as {code?:string}).code;if(code==='AIM_MISSED'||code==='TARGET_UNAVAILABLE')notice.value='Target already cleared. Aim at another creature.';else stopAuto((e as Error).message);}}finally{finishedShot(shot.requestKey);}
}
...
onMounted(async()=>{
 try{
  ...
  if(stage.enabled&&props.authenticated&&session.current){walletPresentation=new FishWalletPresentation(session.current.wallet);holdCredits('reef-party',session.current.id,session.current.wallet.available);}
  app.ticker.add(ticker=>{
   ...
   if(walletPresentation&&session.current){const displayed=walletPresentation.reconcile(session.current.wallet,performance.now(),!firing.value&&!stage.fishPending.length);holdCredits('reef-party',session.current.id,displayed.available);}
...
function cancelFlight(){for(const shot of projectiles){shot.view.destroy();finishedShot(shot.requestKey);}projectiles.length=0;}
...
onBeforeUnmount(()=>{clearTimeout(bannerTimer);stopAuto();disposed=true;revealCredits('reef-party');clearInterval(poll);observer?.disconnect();if(app?.renderer){app.canvas.removeEventListener('pointerdown',fire);app.canvas.removeEventListener('pointermove',aimAt);app.destroy(true,{children:true});if(art)Object.values(art).flat().forEach((t:RenderTexture)=>t.destroy(true));}});
</script>
<template><section class="reef-preview reef-deluxe"><div ref="host" class="fish-canvas" :data-target="lockedTarget ?? ''" :data-shots-fired="firedCount"><p v-if="error" class="error" role="alert">{{error}}</p><div v-if="!running" class="paused-overlay">Paused</div></div><div class="reef-seat-plaque" :data-seat="seat" v-for="seat in 4" :key="seat" :class="[`seat-${seat}`,{yours:room?.seat===seat}]"><small>{{room?.seat===seat?'YOUR CANNON':`CANNON ${seat}`}}</small><b>{{room?.seats.find(s=>s.seat===seat)?.display_name || 'Open seat'}}</b><em v-if="room?.seat===seat && stage.enabled">{{formatCredits(stage.stake)}} / SHOT</em></div><Transition name="catch-banner"><div v-if="catchBanner" class="reef-catch-banner" role="status"><small>{{catchBanner.title}}</small><strong>+{{catchBanner.award}}</strong><span>{{catchBanner.species}} · PLAY CREDITS</span></div></Transition><p class="reef-shot-notice" role="status">{{notice}}</p><div class="reef-console"><div class="led-meter"><small>CREDITS</small><strong>{{balance||'0.00'}}</strong></div><BetControls :reduced-motion="reducedMotion" game="reef-party" :ready="running" :authenticated="!!authenticated" :busy="firing" continuous/><div class="fish-actions"><button :aria-pressed="autoOn" aria-label="Auto fire" :disabled="!running||!!stage.pending||!!error" @click="toggleAuto"><b>⟳</b><span>AUTO {{autoOn?'ON':'OFF'}}</span></button><button :aria-pressed="lockOn" aria-label="Target lock" :disabled="!running" @click="toggleLock"><b>⌖</b><span>LOCK {{lockOn?'ON':'OFF'}}</span></button><button :aria-pressed="fast" aria-label="Fast fire cadence" :disabled="!running" @click="fast=!fast"><b>▶▶</b><span>{{fast?'FAST 2×':'NORMAL 1×'}}</span></button></div><span class="cannon-ready" :class="{busy:firing}">{{firing?'● FIRING':'● CANNON READY'}}</span><button class="line-map-button" :aria-expanded="guide" @click="guide=!guide">SPECIES</button></div><div v-if="guide" class="reef-field-guide" aria-label="Fish species"><span v-for="(species,index) in reefSpecies" :key="species"><AquaticSprite :species="index"/><b>{{species}}</b><small>{{reefTierProfile.tiers[reefTier(index)].label}} · {{reefTierProfile.tiers[reefTier(index)].multiplier}}× shot</small></span></div></section></template>
