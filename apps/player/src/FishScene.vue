...
import {Application,Container,Graphics,Sprite,Text,type RenderTexture} from 'pixi.js';
...
const creatures:{id:number;view:Container;sprite:Sprite;hit:number;captured:boolean}[]=[],cannons:{view:Container;barrel:Sprite;recoil:number;angle:number}[]=[],effects:{view:Container;age:number;life:number;kind:string;vx:number;vy:number}[]=[];
...
async function settleShot(shot:NonNullable<typeof projectile>){
 ...
 impact(flight.x,flight.y);notice.value='Impact';
 ...
 try{
  ...
  if(result.captured){if(fish){fish.captured=true;fish.view.visible=false;}impact(flight.x,flight.y,true);
...
function fireCannon(){
 if(!app||!props.running||firing.value||disposed||stage.busy||stage.pending||props.authenticated&&!room.value)return;
 ...
 projectile={view,flight,firedAt,roomId:room.value?.id,requestKey:crypto.randomUUID(),age:0};firing.value=true;cannon.recoil=.2;notice.value='';
...
onMounted(async()=>{
 try{
  ...
  world=new Container();app.stage.addChild(world);host.value!.appendChild(app.canvas);app.canvas.setAttribute('aria-label','Illustrated reef with eight aquatic species');app.canvas.setAttribute('role','img');
  for(let id=1;id<=80;id++){
   const p=reefTarget(id,0),view=new Container(),sprite=new Sprite(art.creatures[p.species]);sprite.anchor.set(.5);sprite.scale.set(p.radius*2.8/art.creatures[p.species].width);
   const shadow=new Sprite(art.creatures[p.species]);shadow.anchor.set(.5);shadow.scale.copyFrom(sprite.scale);shadow.tint=0x002641;shadow.alpha=.34;shadow.position.set(4,8);view.addChild(shadow,sprite);world.addChild(view);creatures.push({id,view,sprite,hit:0,captured:false});
  }
  ...
  app.ticker.add(ticker=>{
   ...
   for(const fish of creatures){const p=reefTarget(fish.id,time);fish.hit=Math.max(0,fish.hit-dt);fish.view.position.set(p.x+(props.reducedMotion?0:Math.sin(fish.hit*50)*fish.hit*14),p.y);fish.view.scale.set(p.direction,Math.min(1,world.scale.x/world.scale.y));fish.sprite.tint=fish.hit>.2?0xc6f5ff:0xffffff;
    fish.view.rotation=props.reducedMotion?0:Math.sin(time*2+fish.id)*.035;fish.sprite.scale.y=p.radius*2.8/384*(props.reducedMotion?1:1+Math.sin(time*5+fish.id)*.035);
   }
   ...
   for(let i=effects.length-1;i>=0;i--){const e=effects[i];e.age+=dt;e.view.alpha=Math.max(0,1-e.age/e.life);if(e.kind==='particle'){e.view.x+=e.vx*dt;e.view.y+=e.vy*dt;}else e.view.scale.set(props.reducedMotion?1:.55+e.age/e.life*.8);if(e.age>=e.life){e.view.destroy({children:true});effects.splice(i,1);}}
...
onBeforeUnmount(()=>{stopAuto();disposed=true;clearInterval(poll);observer?.disconnect();if(app?.renderer){app.canvas.removeEventListener('pointerdown',fire);app.canvas.removeEventListener('pointermove',aimAt);app.destroy(true,{children:true});if(art)Object.values(art).flat().forEach((t:RenderTexture)=>t.destroy(true));}});
</script>
<template><section class="reef-preview reef-deluxe"><div ref="host" class="fish-canvas" :data-target="lockedTarget ?? ''"><p v-if="error" class="error" role="alert">{{error}}</p><div v-if="!running" class="paused-overlay">Paused</div></div><div class="reef-seat-plaque" :data-seat="seat" v-for="seat in 4" :key="seat" :class="[`seat-${seat}`,{yours:room?.seat===seat}]"><small>{{room?.seat===seat?'YOUR CANNON':`CANNON ${seat}`}}</small><b>{{room?.seats.find(s=>s.seat===seat)?.display_name || 'Open seat'}}</b><em v-if="room?.seat===seat && stage.enabled">{{formatCredits(stage.stake)}} / SHOT</em></div><p class="reef-shot-notice" role="status">{{notice}}</p><div class="reef-console"><div class="led-meter"><small>CREDITS</small><strong>{{balance||'0.00'}}</strong></div><BetControls :reduced-motion="reducedMotion" game="reef-party" :ready="running" :authenticated="!!authenticated" :busy="firing"/><div class="fish-actions"><button :aria-pressed="autoOn" aria-label="Auto fire" :disabled="!running||!!stage.pending||!!error" @click="toggleAuto"><b>⟳</b><span>AUTO {{autoOn?'ON':'OFF'}}</span></button><button :aria-pressed="lockOn" aria-label="Target lock" :disabled="!running" @click="toggleLock"><b>⌖</b><span>LOCK {{lockOn?'ON':'OFF'}}</span></button><button :aria-pressed="fast" aria-label="Fast fire cadence" :disabled="!running" @click="fast=!fast"><b>▶▶</b><span>{{fast?'FAST 2×':'NORMAL 1×'}}</span></button></div><span class="cannon-ready" :class="{busy:firing}">{{firing?'● FIRING':'● CANNON READY'}}</span><button class="line-map-button" :aria-expanded="guide" @click="guide=!guide">SPECIES</button></div><div v-if="guide" class="reef-field-guide" aria-label="Fish species"><span v-for="species in reefSpecies" :key="species">{{species}}</span></div></section></template>
