...
export async function reefTextures(app:Application){
 const sources=await Promise.all(['/art/reef-creatures-v6.png','/art/reef-cannons-v6.png','/art/reef-legends-v10.png'].map(url=>Assets.load<Texture>(url)));
 ...
 const cellWidth=sources[2].width/4,cellHeight=sources[2].height/2;
