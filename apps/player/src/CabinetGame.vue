...
<template>
  <section class="slot-cabinet" :class="[game.id, {'cabinet-active':active, 'stage-paused':!running}]" :data-phase="phase" :style="{'--game-color':game.color}">
    ...
    <header class="slot-marquee"><span class="marquee-wing"><ArcadeSymbol :symbol="coinGame?'coin':game.id==='jade-fortune'?'dragon':'seven'"/></span><div><small>NEW GAME ORIGINAL</small><h2>{{ game.name }}</h2><p>{{ game.detail }}</p></div><span class="marquee-wing"><ArcadeSymbol :symbol="coinGame?'coin':game.id==='jade-fortune'?'dragon':'seven'"/></span></header>
    <div class="cabinet-game-body">
      <aside class="cabinet-side left"><span class="rail-stars" aria-hidden="true">✦<br>✦<br>✦</span><div class="cabinet-readout"><small>{{ coinGame ? 'HELD' : 'MATCHES' }}</small><strong>{{ coinGame ? locked.length : phase === 'complete' ? result?.matches.length || 0 : '—' }}<em v-if="coinGame"> / 5</em></strong></div><b>{{ coinGame ? 'COLLECT' : game.id === 'jade-fortune' ? 'WILD' : 'SEVENS' }}</b></aside>
