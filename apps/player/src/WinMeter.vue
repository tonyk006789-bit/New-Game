...
let animation=0;
function settle(){cancelAnimationFrame(animation);counting.value=false;shown.value=props.busy?'0':props.amount;}
watch(()=>[props.amount,props.busy,props.reducedMotion,props.running] as const,(_next,previous)=>{
 ...
 if(props.busy||!previous?.[1]||props.reducedMotion||props.running===false||!positive.value)return;
 const award=BigInt(props.amount),start=performance.now();counting.value=true;shown.value='0';
