...
const credits = computed(() => formatCredits(account.value?.wallet.available || '0'));
...
function clearAccount(){session.current=null;stage.pending=null;stage.needsRecovery=false;stage.last=null;account.value=null;entered.value=false;activeGame.value=null;selectedTable.value=null;atFishTable.value=false;modal.value=null;creditHistory.value=[];practiceHistory.value=[];}
...
const modal = ref<'about' | 'support' | null>(null);
...
function trapFocus(event: KeyboardEvent) {
  ...
  const items = Array.from(document.querySelectorAll<HTMLElement>('.player-modal button:not([disabled]), .player-modal input'));
...
<template>
  <div class="arcade-app" :class="{ 'reduce-motion': reducedMotion, 'in-game': currentGame, 'at-login': !entered }">
    <LoginScreen v-if="!entered" @authenticated="authenticated" @preview="enterPreview" @help="modal = 'support'" />
    <template v-else>
      ...
      <main v-else class="utility-page">
        ...
        <template v-else><h1>YOUR SETTINGS</h1><p class="utility-lead">Make the arcade feel right for you.</p><section class="arcade-panel settings-panel"><div><span><strong>Reduced motion</strong><p>Calmer scenes and instant reel previews.</p></span><button class="toggle" :class="{ on: reducedMotion }" role="switch" :aria-checked="reducedMotion" aria-label="Reduced motion" @click="reducedMotion = !reducedMotion"><i></i></button></div><div><span><strong>Sound</strong><p>The current preview is silent. Music and effects controls will arrive with audio.</p></span><span class="scene-preview-label">SILENT</span></div><div><span><strong>{{account ? 'Account access' : 'Preview access'}}</strong><p>{{account ? `Signed in as ${account.username}.` : 'You are exploring as a guest.'}}</p></span><button class="back-text" @click="leavePreview">{{account ? 'SIGN OUT' : 'LOGIN'}} <Icon name="arrow" :size="16" /></button></div></section></template>
      </main>
    ...
    <div v-if="modal" class="player-modal-backdrop" @click.self="modal=null"><section class="player-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" @keydown="trapFocus">
      <header><h2 id="modal-title">{{modal==='support'?'NEED A HAND?':'WELCOME TO NEW GAME'}}</h2><button class="round-control" aria-label="Close dialog" @click="modal=null"><Icon name="close"/></button></header>
      <template v-if="modal==='support'"><p>For your player ID, an invitation, a password reset, or help with credits, contact your group’s Main Admin through your usual contact method.</p><p>Sign in with the player ID and password provided by your administrator. Guest previews are also available.</p></template>
      ...
      <button class="gold-button" @click="modal=null">GOT IT <Icon name="check" :size="17"/></button>
    </section></div>
  </div>
</template>
