...
export function validateHostedTest(env: NodeJS.ProcessEnv = process.env) {
 ...
 if(!/^[a-f0-9-]{36}$/i.test(env.HOSTED_TEST_SITE_ID || '') || env.SITE_ID !== env.HOSTED_TEST_SITE_ID)throw new Error('Hosted test must be bound to its dedicated Netlify project.');
